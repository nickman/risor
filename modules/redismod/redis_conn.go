...
func New(ctx context.Context, client BaseClient) *RedisConn {
	...
	obj.waitToClose()
	serverInfo := client.Info(ctx, "server").Val()
	obj.version = indexInfo(serverInfo)["redis_version"]
	runtime.SetFinalizer(obj, func(r *RedisConn) {
		_ = r.Close()
	})
	return obj
}
...
func (c *RedisConn) GetAttr(name string) (object.Object, bool) {
	switch name {
	case "version":
		return object.NewString(c.version), true
	case "close":
		fmt.Printf("Closing...\n")
	...
	if attr, ok := c.HashCmdsGetAttr(name); ok {
		return attr, ok
	}
	...
	if attr, ok := c.BitCmdsGetAttr(name); ok {
		return attr, ok
	}
...
func indexInfo(info string) map[string]string {
